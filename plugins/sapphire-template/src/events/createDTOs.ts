...
export const afterCreateDTOs = (
  ...
  modules: Module[]
): Module[] => modules.map((module: Module) => {
  ...
  visit(template, {
    visitImportDeclaration(path) {
      ...
      if (/(?:\.\.\/\.\.\/+)|(?:\/base\/+)/g.test(path.value.source.value)) {
        ...
        const manipulatedPath = splitPath.reduce((finalPath: string, stringPath: string) => {
          const addedStr = stringPath.includes("base") ? "model/dtos" : stringPath
          finalPath += `${addedStr}/`;
