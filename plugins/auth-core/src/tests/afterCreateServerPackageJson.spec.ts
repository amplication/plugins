...
describe("Testing afterCreateServerPackageJson hook", () => {
    ...
    it("should create the grants module when the context has entities and roles specified", async () => {
        const updatedModules = await plugin.afterCreateServerPackageJson(context, params, modules);
        const grantsModule = prettyCode(updatedModules.get("src/grants.json")?.code);
