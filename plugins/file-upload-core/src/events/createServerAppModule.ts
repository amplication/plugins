...
const prepareStorageModuleTemplate = async (
  ...
  // Gets name of the file-upload plugins that are enabled ( for this to work the plugins must have the file-upload prefix)
  const fileUploadPlugins = pluginInstallations
    .filter(
      (plugin) =>
        plugin.pluginId.includes("file-upload") &&
        plugin.enabled &&
        plugin.pluginId !== "file-upload-core",
    )
