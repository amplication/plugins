...
export const secretsManagerFactory = {
    ...
    useFactory: async (configService: ConfigService) => {
        ...
        if (!auth.success) {
            logger.error(`Bitwarden authentication failed: ${auth.errorMessage}`,
                "SecretsManager")
        ...
        if (!secretsList.success) {
            logger.error(`Error while fetching secrets : ${secretsList.errorMessage}`,
                "SecretsManager")
        ...
        secretsList.data?.data.forEach((sec: SecretIdentifierResponse) => {
            if (Object.values(EnumSecretsNameKey).includes(sec.key as EnumSecretsNameKey)) {
        ...
        if (response.success) {
            logger.error(`Error fetching secrets from secretManager :
                ${response.errorMessage}`, "SecretsManager")
