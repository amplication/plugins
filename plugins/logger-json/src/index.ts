...
class LoggerJSONPlugin implements AmplicationPlugin {
    ...
    beforeCreateServerDotEnv(context: DsgContext, eventParams: CreateServerDotEnvParams) {
        const { logLevel: LOG_LEVEL } = getPluginSettings(context.pluginInstallations)
    ...
    async afterCreateServer(context: DsgContext, params: CreateServerParams, modules: ModuleMap) {
        const { additionalLogProperties: extras } = getPluginSettings(context.pluginInstallations)

        // Use custom logger in main.ts
        useLoggerInMain(context.serverDirectories.srcDirectory, modules, context.logger)
        ...
        // Copy templates
        const template = await readFile(resolve(__dirname, "templates", "logger.config.ts"))
        ...
        await modules.set({
            ...
            path: join(context.serverDirectories.srcDirectory, "logger", "logger.config.ts")
        })
        
        return modules
    }

    beforeCreateServerPackageJSON(_: DsgContext, eventParams: CreateServerPackageJsonParams) {
        eventParams.updateProperties.push(dependencies)
        
        return eventParams
    }

    beforeCreateServerAppModule(_: DsgContext, eventParams: CreateServerAppModuleParams) {
    ...
    async afterCreateServerAppModule(context: DsgContext, _: CreateServerAppModuleParams, modules: ModuleMap) {
        ...
        const loggerModuleImport = importNames([loggerModuleId], "./logger/logger.module")
