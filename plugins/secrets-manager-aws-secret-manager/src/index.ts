...
class ExamplePlugin implements AmplicationPlugin {
    ...
    async beforeCreateServer(context: DsgContext, _: CreateServerParams, modules: ModuleMap): Promise<ModuleMap> {
        const { fetchMode, secretNames } = getPluginSettings(context.pluginInstallations)
