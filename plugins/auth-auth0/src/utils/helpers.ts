...
export const getSearchableAuthField = (entity: Entity, recipe: IRecipe) : EntityField => {
  ...
  if(!authEmailField) {
    throw new Error(`The entity ${entity.name} does not have a field named ${emailField || payloadEmailField || fallbackEmailField?.name}`);
  } 
