...
export const beforeCreateAdminDotEnv = async (
  ...
  if(useManagementApi) {
    ...
    ({ audience, clientID, domain } = await Auth0Environment.getInstance({ ...managementParams, jwtToken: managementParams.accessToken, logger: context.logger}));
  }

  if(!domain || !clientID || !audience) {
    context.logger.error("Domain, Client ID and Audience are required.", {}, "Kindly add the Domain, Client ID and Audience in the plugin settings or use the Management API by providing the Management API Access Token and Identifier in the plugin settings' managementParams field, and make useManagementApi true.");
    throw new Error("Auth0 environment creation failed.");
  }
