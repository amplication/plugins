import { CreateSeedParams, DsgContext, Entity, EntityField, EnumDataType, types } from "@amplication/code-gen-types";
import { join } from "path";
import { AUTH_ENTITY_ERROR, AUTH_ENTITY_LOG_ERROR, templatesPath } from "../constants";
...
export const DEFAULT_ROLE_LITERAL = builders.arrayExpression([builders.stringLiteral("user")]);
...
export const beforeCreateSeed = async (
  ...
  const seedProperties = createAuthEntityObjectCustomProperties(authEntity, defaultUser);
...
export function createDefaultValue(
  ...
  switch (field.dataType) {
    ...
    case EnumDataType.MultiLineText: {
      return defaultValue ? builders.stringLiteral(defaultValue as string) : DEFAULT_EMPTY_STRING_LITERAL;
    }
    case EnumDataType.Email: {
      return defaultValue ? builders.stringLiteral(defaultValue as string) : DEFAULT_EMAIL_LITERAL;
    }
    case EnumDataType.WholeNumber: {
      return defaultValue ? builders.numericLiteral(defaultValue as number) : DEFAULT_NUMBER_LITERAL;
    }
    ...
    case EnumDataType.OptionSet: {
      ...
      return defaultValue ? builders.stringLiteral(defaultValue as string) : memberExpression`${createEnumName(
        field,
        entity
      )}.${pascalCase(firstOption.label)}`;
    }
    case EnumDataType.Boolean: {
      return defaultValue ? builders.booleanLiteral(defaultValue as boolean) : DEFAULT_BOOLEAN_LITERAL;
    }
    case EnumDataType.GeographicLocation: {
      return defaultValue ? builders.stringLiteral(defaultValue as string) : DEFAULT_ADDRESS_LITERAL;
    }
    ...
    case EnumDataType.Username: {
      return defaultValue ? builders.stringLiteral(defaultValue as string) : DEFAULT_USERNAME_LITERAL;
    }
    case EnumDataType.Roles: {
      return defaultValue ? builders.arrayExpression((defaultValue as string[]).map(item => builders.stringLiteral(item))) : DEFAULT_ROLE_LITERAL;
    }
