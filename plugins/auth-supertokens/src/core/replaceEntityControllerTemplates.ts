import { CreateEntityControllerBaseParams, CreateEntityControllerParams, Entity } from "@amplication/code-gen-types";
...
export const replaceEntityControllerBaseTemplate = async (
    ...
    const templatePath = resolve(templatesPath, "authEntity.controller.base.template.ts"); 
...
export const replaceEntityControllerTemplate = async (
    eventParams: CreateEntityControllerParams
) => {
    const templatePath = resolve(templatesPath, "authEntity.controller.template.ts"); 
    eventParams.template = await readFile(templatePath);
}
