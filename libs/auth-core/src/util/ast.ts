...
function consolidateImports(
  ...
  const moduleToDeclarations = groupBy(
    declarations,
    (declaration: { source: { value: any } }) => declaration.source.value
  );
  const moduleToDeclaration = mapValues(
    moduleToDeclarations,
    (declarations: any[], module: string) => {
      const specifiers = uniqBy(
        declarations.flatMap(
          (declaration: { specifiers: any }) => declaration.specifiers || []
        ),
        (specifier: { imported: { name: any }; type: any }) => {
...
export function importContainedIdentifiers(
  ...
  const moduleToContainedIds = groupBy(
    containedIds,
